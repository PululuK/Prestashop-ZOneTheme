...
class ZOneThemeManager extends Module
{
    ...
    {
        $sql = 'DESCRIBE `'._DB_PREFIX_.'zthememanager_lang`';        
        $columns = Db::getInstance()->executeS($sql);
        $found = false;
        foreach ($columns as $col) {
            if ($col['Field'] == 'cookie_message') {
                $found = true;
                break;
            }
        }
        ...
        if (!$found) {
            $sql = 'ALTER TABLE `'._DB_PREFIX_.'zthememanager_lang` ADD `cookie_message` TEXT NULL AFTER `footer_bottom`';
    ...
    {
        if (!$this->isCached('zonethememanager_outsidemainpage.tpl', $this->getCacheId())) {
            $id_lang = (int) $this->context->language->id;
            $settings = ZManager::getSettingsByShop($id_lang);
        ...
        return $this->display(__FILE__, 'zonethememanager_outsidemainpage.tpl', $this->getCacheId());
    }
}
